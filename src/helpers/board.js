...
const getEntrySlot = (colId) =>
	document.querySelectorAll(".board__entry")[colId];
...
const handleClick = (colId, boardState, updateBoardState, playerId) => {
	...
	!entrySlot.classList.contains(`player-${playerId}`) &&
		entrySlot.classList.add(`player-${playerId}`);
...
const playTurn = (boardState, updateBoardState, playerId) => {
	const availableRows = boardState.top
		.map((top, idx) => (top > 0 ? idx : 0))
		.filter((top) => !!top);
...
const getValidCols = (boardState) =>
	boardState.top
		.map((top, idx) => (top >= 0 ? idx : -1))
		.filter((top) => top >= 0);

const getRandomCol = (availableRows) =>
